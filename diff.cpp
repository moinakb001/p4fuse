...
class MyClientUser : public ClientUser
{
    ...
    virtual void OutputInfo( char level, const char *data) override
    {
            ...
            (this->type == ctype_e::files && level == '1' && strncmp(data, "depotFile ", sizeof("depotFile ")-1) == 0))
        {
            ...
            u64 len = strlen(d);
            u32 node = find_or_insert_path<gc_node>(table, id, (u8*)d, len);
            
            auto ent = &table->entries[node];
            if((node <= table->id && ent->gen == table->gen) || (node > table->id && ent->gen + 1 == table->gen))
            {
            ent->bList = 0;
            ent->parent = id;
            ent->bDir = this->type == ctype_e::dirs;
            ent->left = 0;
            ent->right = 0;
            }
            else printf("WHAT THE FUCK\n");
            this->node = node;
            __builtin_memcpy(node_name(table, ent), d, len);
        }
        else if(this->type == ctype_e::sync || this->type == ctype_e::files){
            printf("aux %s\n", data);
        }
        if(this->type == ctype_e::files && level == '1' && strncmp(data, "fileSize ", sizeof("fileSize ")-1) == 0)
        {
            sscanf(data, "fileSize %llu", &table->entries[node].size);
        }
...
getattr_out:
            ...
            case FUSE_OPEN:
            {
                ...
                if(table->entries[hdr.nodeid].opened_file == 0)
                {
                    path[0] = 'a';
                    ...
                    while(files < 0)
                    {
                        ...
                        buf[mad] = 0;
                        printf("strs %s\n", buf);
                        files = open((char*)buf,O_RDWR | O_NOFOLLOW);
                    }
            ...
            case FUSE_LOOKUP:
            {
             ...
             //   iout->attr_valid = 1llu << 60;
               ...
               *iout = fuse_entry_out{};
                ...
                iout->attr.mode |= table->entries[n].bDir ? S_IFDIR : (S_IFREG | 0111);
                num_rqs++;
                send_message(outh);
                cur += hdr.len;
                break;
